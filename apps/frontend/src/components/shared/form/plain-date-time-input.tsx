...
const YEARS = Array.from({ length: YEAR_END - 1900 + 1 }, (_, i) => 1900 + i);

export function PlainDateTimeInput({
  value,
  onChange,
  mode = "datetime-local",
  placeholder,
  ...
  id,
  disabled,
}: PlainDateTimeInputProps) {
  const selectedDate = value ? new Date(value) : null;
  const isDateTime = mode === "datetime-local";
  ...
  return (
    <DatePicker
      ...
      dateFormat={isDateTime ? "dd MMM yyyy, hh:mm aa" : "dd MMM yyyy"}
      timeFormat="HH:mm"
      timeIntervals={15}
      placeholderText={
        placeholder ?? (isDateTime ? "Select date and time" : "Select date")
      }
      disabled={disabled}
      id={id}
